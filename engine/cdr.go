...
func NewCDRFromExternalCDR(extCdr *ExternalCDR, timezone string) (*CDR, error) {
	var err error
	cdr := &CDR{UniqueID: extCdr.UniqueID, RunID: extCdr.RunID, OrderID: extCdr.OrderID, ToR: extCdr.ToR, OriginID: extCdr.OriginID, OriginHost: extCdr.OriginHost,
...
func NewCDRWithDefaults(cfg *config.General) *CDR {
	return &CDR{ToR: utils.VOICE, RequestType: *cfg.DefaultRequestType, Direction: utils.OUT, Tenant: *cfg.DefaultTenant, Category: *cfg.DefaultCategory,
		ExtraFields: make(map[string]string), Cost: dec.NewVal(-1, 0)}
}
...
type CDR struct {
	...
	RequestType     string            // matching the supported request types by the **accuRate**, accepted values are hardcoded in the server <prepaid|postpaid|pseudoprepaid|rated>.
	...
	PDD             time.Duration     // PDD value (post-dial delay)
...
func (cdr *CDR) UsageMultiply(multiplyFactor float64, roundDecimals int) {
	ns := dec.NewVal(cdr.Usage.Nanoseconds(), 0).MulS(dec.NewFloat(multiplyFactor)).Round(int32(roundDecimals))
	cdr.Usage = time.Duration(ns.Int64())
}

// Used to multiply cost on export
func (cdr *CDR) CostMultiply(multiplyFactor float64, roundDecimals int) {
	cdr.GetCost().MulS(dec.NewFloat(multiplyFactor)).Round(int32(roundDecimals))
}

// Format cost as string on export
func (cdr *CDR) FormatCost(shiftDecimals, roundDecimals int) string {
	cost := dec.New().Set(cdr.GetCost())
	if shiftDecimals != 0 {
		cost.MulS(dec.NewFloat(math.Pow10(shiftDecimals)))
	}
	return cost.Round(int32(roundDecimals)).String()
}
...
func (cdr *CDR) FormatUsage(layout string) string {
	if utils.IsSliceMember([]string{utils.DATA, utils.SMS, utils.MMS, utils.GENERIC}, cdr.ToR) {
		return dec.NewFloat(cdr.Usage.Seconds()).String()
	}
	switch layout {
	default:
		return strconv.FormatFloat(float64(cdr.Usage.Nanoseconds())/1000000000, 'f', -1, 64) //dec.NewVal(cdr.Usage.Nanoseconds(), 0).QuoS(dec.NewVal(1, -9)).String() // TODO:  why not seconds?
	}
}
...
func (cdr *CDR) FieldAsString(rsrFld *utils.RSRField) string {
	...
	switch rsrFld.Id {
	case utils.UniqueID:
		return rsrFld.ParseValue(cdr.UniqueID)
	case utils.ORDERID:
	...
	case utils.COST:
		return rsrFld.ParseValue(cdr.GetCost().String()) // Recommended to use FormatCost
...
func (cdr *CDR) ParseFieldValue(fieldId, fieldVal, timezone string) error {
	...
	case utils.COST:
		if cdr.Cost, err = dec.New().SetString(fieldVal); err != nil {
			return err
		}
	case utils.PartialField:
...
func (cdr *CDR) ForkCdr(runId string, RequestTypeFld, directionFld, tenantFld, categFld, accountFld, subjectFld, destFld, setupTimeFld, PDDFld,
	...
	var err error
	frkStorCdr := new(CDR)
	frkStorCdr.UniqueID = cdr.UniqueID
	frkStorCdr.ToR = cdr.ToR
	frkStorCdr.RunID = runId
	frkStorCdr.Cost = dec.NewVal(-1, 0) // Default for non-rated CDR
	...
	} else if frkStorCdr.Cost, err = dec.New().SetString(costStr); err != nil {
		return nil, err
	}
...
func (cdr *CDR) AsExternalCDR() *ExternalCDR {
	return &ExternalCDR{UniqueID: cdr.UniqueID,
		RunID:           cdr.RunID,
		OrderID:         cdr.OrderID,
...
func (cdr *CDR) GetUniqueID(timezone string) string {
	return cdr.UniqueID
}
...
func (cdr *CDR) combimedCdrFieldVal(cfgCdrFld *config.CdrField, groupCDRs []*CDR) (string, error) {
	...
	for _, filterRule := range cfgCdrFld.FieldFilter {
		...
		for _, grpCDR := range groupCDRs {
			if cdr.UniqueID != grpCDR.UniqueID {
...
func (cdr *CDR) exportFieldValue(cfgCdrFld *config.CdrField) (string, error) {
	passesFilters := true
	...
	for _, rsrFld := range cfgCdrFld.Value {
		...
		case utils.DESTINATION:
			...
			if cfgCdrFld.MaskLen != -1 && len(cfgCdrFld.MaskDestID) != 0 && CachedDestHasPrefix(cdr.Tenant, cfgCdrFld.MaskDestID, cdrVal) {
...
func (cdr *CDR) formatField(cfgFld *config.CdrField, httpSkipTlsCheck bool, groupedCDRs []*CDR) (fmtOut string, err error) {
	...
	case utils.MetaMaskedDestination:
		if len(cfgFld.MaskDestID) != 0 && CachedDestHasPrefix(cdr.Tenant, cfgFld.MaskDestID, cdr.Destination) {
			outVal = "1"
		} else {
			outVal = "0"
		}
...
func (cdr *CDR) AsExportRecord(exportFields []*config.CdrField, httpSkipTlsCheck bool, groupedCDRs []*CDR) (expRecord []string, err error) {
...
func (cdr *CDR) AsExportMap(exportFields []*config.CdrField, httpSkipTlsCheck bool, groupedCDRs []*CDR) (expMap map[string]string, err error) {
...
func (cdr *CDR) NeedsUsers() bool {
	if cdr.UniqueID == utils.USERS {
		return true
	}
	if cdr.RunID == utils.USERS {
		return true
	}
	if cdr.OriginHost == utils.USERS {
		return true
	}
	if cdr.Source == utils.USERS {
		return true
	}
	if cdr.OriginID == utils.USERS {
		return true
	}
	if cdr.ToR == utils.USERS {
		return true
	}
	if cdr.RequestType == utils.USERS {
		return true
	}
	if cdr.Direction == utils.USERS {
		return true
	}
	if cdr.Tenant == utils.USERS {
		return true
	}
	if cdr.Category == utils.USERS {
		return true
	}
	if cdr.Account == utils.USERS {
		return true
	}
	if cdr.Subject == utils.USERS {
		return true
	}
	if cdr.Destination == utils.USERS {
		return true
	}
	if cdr.Supplier == utils.USERS {
		return true
	}
	if cdr.DisconnectCause == utils.USERS {
		return true
	}
	if cdr.CostSource == utils.USERS {
		return true
	}
	if cdr.ExtraInfo == utils.USERS {
		return true
	}
	for _, value := range cdr.ExtraFields {
...
func (ecdr *ExternalCDR) NeedsUsers() bool {
	if ecdr.UniqueID == utils.USERS {
		return true
	}
	if ecdr.RunID == utils.USERS {
		return true
	}
	if ecdr.OriginHost == utils.USERS {
		return true
	}
	if ecdr.Source == utils.USERS {
		return true
	}
	if ecdr.OriginID == utils.USERS {
		return true
	}
	if ecdr.ToR == utils.USERS {
		return true
	}
	if ecdr.RequestType == utils.USERS {
		return true
	}
	if ecdr.Direction == utils.USERS {
		return true
	}
	if ecdr.Tenant == utils.USERS {
		return true
	}
	if ecdr.Category == utils.USERS {
		return true
	}
	if ecdr.Account == utils.USERS {
		return true
	}
	if ecdr.Subject == utils.USERS {
		return true
	}
	if ecdr.Destination == utils.USERS {
		return true
	}
	if ecdr.SetupTime == utils.USERS {
		return true
	}
	if ecdr.PDD == utils.USERS {
		return true
	}
	if ecdr.AnswerTime == utils.USERS {
		return true
	}
	if ecdr.Usage == utils.USERS {
		return true
	}
	if ecdr.Supplier == utils.USERS {
		return true
	}
	if ecdr.DisconnectCause == utils.USERS {
		return true
	}
	if ecdr.CostSource == utils.USERS {
		return true
	}
	if ecdr.CostDetails == utils.USERS {
		return true
	}
	if ecdr.ExtraInfo == utils.USERS {
		return true
	}
	for _, value := range ecdr.ExtraFields {
		if value == utils.USERS {
			return true
		}
	}
...
func (ur *UsageRecord) AsStoredCdr(timezone string) (*CDR, error) {
	var err error
	cdr := &CDR{UniqueID: ur.GetId(), ToR: ur.ToR, RequestType: ur.RequestType, Direction: ur.Direction, Tenant: ur.Tenant, Category: ur.Category, Account: ur.Account, Subject: ur.Subject, Destination: ur.Destination}
	if cdr.SetupTime, err = utils.ParseTimeDetectLayout(ur.SetupTime, timezone); err != nil {
		return nil, err
	}
	if cdr.AnswerTime, err = utils.ParseTimeDetectLayout(ur.AnswerTime, timezone); err != nil {
		return nil, err
	}
	if cdr.Usage, err = utils.ParseDurationWithSecs(ur.Usage); err != nil {
		return nil, err
	}
...
func (ur *UsageRecord) AsCallDescriptor(timezone string, denyNegative bool) (*CallDescriptor, error) {
	...
	cd := &CallDescriptor{
		...
		// denyNegative not implemented (check cgr)
	}
	...
	if usage, err := utils.ParseDurationWithSecs(ur.Usage); err != nil {
		return nil, err
	} else {
...
func (ur *UsageRecord) GetId() string {
	return utils.Sha1(ur.ToR, ur.RequestType, ur.Direction, ur.Tenant, ur.Category, ur.Account, ur.Subject, ur.Destination, ur.SetupTime, ur.AnswerTime, ur.Usage)
}

func (ur *UsageRecord) NeedsUsers() bool {
	if ur.ToR == utils.USERS {
		return true
	}
	if ur.RequestType == utils.USERS {
		return true
	}
	if ur.Direction == utils.USERS {
		return true
	}
	if ur.Tenant == utils.USERS {
		return true
	}
	if ur.Category == utils.USERS {
		return true
	}
	if ur.Account == utils.USERS {
		return true
	}
	if ur.Subject == utils.USERS {
		return true
	}
	if ur.Destination == utils.USERS {
		return true
	}
	if ur.SetupTime == utils.USERS {
		return true
	}
	if ur.AnswerTime == utils.USERS {
		return true
	}
	if ur.Usage == utils.USERS {
		return true
	}
	for _, value := range ur.ExtraFields {
		if value == utils.USERS {
			return true
		}
	}
