...
func TestCDRFields(t *testing.T) {
	fsCdrCfg.Cdrs.ExtraFields = []*utils.RSRField{&utils.RSRField{Id: "sip_user_agent"}}
	fsCdr, err := NewFSCdr(body, fsCdrCfg)
	if err != nil {
		t.Errorf("Error loading cdr: %v", err)
	}
	...
	expctCDR := &CDR{UniqueID: "164b0422fdc6a5117031b427439482c6a4f90e41", ToR: utils.VOICE, OriginID: "e3133bf7-dcde-4daf-9663-9a79ffcef5ad",
		OriginHost: "127.0.0.1", Source: "freeswitch_json", Direction: utils.OUT, Category: "call", RequestType: utils.META_PREPAID, Tenant: "accurate", Account: "1001", Subject: "1001",
		...
		DisconnectCause: "NORMAL_CLEARING", ExtraFields: map[string]string{"sip_user_agent": "PJSUA v2.3 Linux-3.2.0.4/x86_64/glibc-2.13"}, Cost: dec.NewFloat(-1)}
...
func TestSearchExtraField(t *testing.T) {
	...
	fsCdrCfg.Cdrs.ExtraFields = []*utils.RSRField{&utils.RSRField{Id: "caller_id_name"}, rsrSt1, rsrSt2}
...
func TestSearchReplaceInExtraFields(t *testing.T) {
	fsCdrCfg.Cdrs.ExtraFields = []*utils.RSRField{&utils.RSRField{Id: "read_codec"},
...
"cdrs": {
	...
	config.Reset()
	fsCdrCfg = config.Get()
	err = config.LoadBytes(eFieldsCfg, true)
	expectedEF := utils.RSRFields{&utils.RSRField{Id: "effective_caller_id_number",
		RSRules: []*utils.ReSearchReplace{&utils.ReSearchReplace{SearchRegexp: regexp.MustCompile(`(\d+)`), ReplaceTemplate: "+$1"}}}}
	if err != nil {
		t.Error("Could not parse the config", err.Error())
	} else if !reflect.DeepEqual(fsCdrCfg.Cdrs.ExtraFields.R(), expectedEF) {
		t.Errorf("Unexpected value for config CdrsExtraFields: %s %s", utils.ToIJSON(fsCdrCfg.Cdrs.ExtraFields), utils.ToIJSON(expectedEF))
	}
