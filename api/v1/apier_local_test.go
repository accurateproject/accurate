...
func TestApierCreateDirs(t *testing.T) {
	for _, pathDir := range []string{*(*cfg.Cdre)[utils.META_DEFAULT].ExportDirectory, "/var/log/cgrates/cdrc/in", "/var/log/cgrates/cdrc/out", *cfg.Historys.HistoryDir} {
...
func TestApierExportCdrsToFile(t *testing.T) {
    ...
    expectReply := &utils.ExportedFileCdrs{ExportedFilePath: utils.CDRE_DRYRUN, TotalRecords: 2, ExportedUniqueIDs: []string{utils.Sha1("dsafdsaf", tm1.String()),
...
func TestApierLocalProcessCdr(t *testing.T) {
	var reply string
	cdr := engine.CDR{UniqueID: utils.Sha1("dsafdsaf", time.Date(2013, 11, 7, 8, 42, 26, 0, time.UTC).String()), OrderID: 123, ToR: utils.VOICE, OriginID: "dsafdsaf",
