...
func (self *ApierV1) RemoveActionTrigger(attr AttrRemoveActionTrigger, reply *string) error {
	...
	if attr.UniqueID == "" {
		...
		atrs, err := self.RatingDb.GetActionTriggers(attr.GroupID, utils.CACHED)
		if err != nil {
			*reply = err.Error()
			return err
		}
...
func (self *ApierV1) SetActionTrigger(attr AttrSetActionTrigger, reply *string) error {
	...
	atrs, _ := self.RatingDb.GetActionTriggers(attr.GroupID, utils.CACHED)
...
func (self *ApierV1) GetActionTriggers(attr AttrGetActionTriggers, atrs *engine.ActionTriggers) error {
	...
	if len(attr.GroupIDs) > 0 {
		for _, key := range attr.GroupIDs {
			getAttrs, err := self.RatingDb.GetActionTriggers(key, utils.CACHED)
			if err != nil {
				return err
			}
		...
		for _, key := range keys {
			getAttrs, err := self.RatingDb.GetActionTriggers(key[len(utils.ACTION_TRIGGER_PREFIX):], utils.CACHED)
			if err != nil {
				return err
			}
			allAttrs = append(allAttrs, getAttrs...)
