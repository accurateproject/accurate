...
var tutAstCallsCfg *config.Config
...
func TestTutAstCallsInitCfg(t *testing.T) {
	...
	var err error
	config.Reset()
	if err = config.LoadPath(path.Join(*dataDir, "tutorials", "asterisk_events", "cgrates", "etc", "cgrates")); err != nil {
		t.Error(err)
	}
	tutAstCallsCfg = config.Get()
	tutAstCallsCfg.General.DataFolderPath = *dataDir // Share DataFolderPath through config towards StoreDb for Flush()
}
...
func TestTutAstCallsStartEngine(t *testing.T) {
	...
	engine.KillProcName("cc-engine", *waitRater)
...
func TestTutAstCallsRpcConn(t *testing.T) {
	...
	var err error
	tutAstCallsRpc, err = jsonrpc.Dial("tcp", *tutAstCallsCfg.Listen.RpcJson) // We connect over JSON so we can also troubleshoot if needed
	if err != nil {
		t.Fatal(err)
	}
}
